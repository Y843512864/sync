            ...
            if ((mName = m.getName()).startsWith("set")) { // 只进set方法
                ...
                tohere: for (Field fd : dtofds) {
                    ...
                    if (fd.isAnnotationPresent(RelMapper.class) || dtoExistAnno) { // 判断field上注解或类上面注解是否存在
                        ...
                        dtoMapName = fd.isAnnotationPresent(RelMapper.class)
                                ? (fd.getAnnotation(RelMapper.class).value().toString().equals("")
                                        ? fd.getName().toString()
                                        : fd.getAnnotation(RelMapper.class).value().toString())
                                : fd.getName().toString();
